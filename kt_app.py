    ...
    if st.button('Forecast & Calculate'):
        ...
        if latitude>=0:
            if julian_day>162 and julian_day<180:
                brw = (gd/2) +  ((gd/5)*m.cos(m.degrees(wall_azimuth_angle))) +  ((gd/5)*m.cos(m.degrees(wall_azimuth_angle + 190)))
            else:
                brw = (gd/2) + ((gd/5)*m.cos(m.degrees(wall_azimuth_angle))) + (gd/10)*m.cos(m.radians(360*(julian_day - 172)/365))
                
        elif latitude<0:
            brw = (gd/2) + ((gd/5)*m.cos(m.degrees(wall_azimuth_angle + 180)))
